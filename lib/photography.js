...
// ── Coarse areas ────────────────────────────────────────────────────────────
// The gallery used to show nine filter pills, five of which were Canadian
// cities. These are the groupings a visitor actually thinks in.
const AREA_MAP = [
  ['Canada', ['Québec', 'Montréal', 'Toronto', 'Niagara', 'Ontario']],
  ['Kerala', ['Kerala']],
  ['Gujarat', ['Ahmedabad', 'Kutch']],
  ['Mumbai', ['Mumbai']],
];

export function areaOf(photo) {
  for (const [area, regions] of AREA_MAP) {
    if (regions.includes(photo.region)) return area;
  }
  return 'Elsewhere';
}

export const AREAS = AREA_MAP
  .map(([area]) => ({ area, count: PHOTOS.filter((p) => areaOf(p) === area).length }))
  .filter((a) => a.count > 0);
...
// ── URLs ────────────────────────────────────────────────────────────────────
// Preference order: Cloudinary (once uploaded) → committed local preview in
// public/photos → the inline blur. So the gallery shows real images straight
// after `npm run photos`, and silently upgrades to the CDN when it's wired up.
export function photoSrc(photo, transform = 'f_auto,q_auto,w_1600,c_limit') {
  ...
  if (photo?.local) return `/photos/${photo.id}.webp`;
...
export const hasImage = (photo) => Boolean((photo?.uploaded && CLOUD) || photo?.local);
